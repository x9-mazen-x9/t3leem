...
# TIP-03:
# ⚠ cache_page يعتمد على إعدادات CACHES.
# إذا لم يكن Redis مفعّل في الإنتاج (واستخدمت LocMemCache)،
# فالكاش سيكون لكل Worker على حدة (غير مشترك بين Gunicorn workers).
# هذا قد يؤدي إلى:
# - نتائج غير متزامنة بين المستخدمين
# - استهلاك ذاكرة زائد
# لذلك:
# ✔ استخدم Redis في production دائماً
# ✔ LocMemCache مناسب فقط للتطوير
...
class TeacherViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def follow(self, request, pk=None):
        ...
        # ✅ بدون Query إضافية
        if following:
            _check_auto_verify(teacher, followers_count)
...
def _check_auto_verify(teacher, followers_count):
    """
    تحقق تلقائي عند وصول المدرس لـ 5000 متابع.
    بدون تنفيذ Query إضافية.
    """
    if not teacher.is_verified_auto and followers_count >= 5000:
        Teacher.objects.filter(pk=teacher.pk).update(
            is_verified_auto=True
        )
